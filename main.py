...
def generate_inline_selector():
    ...
    inline_btn_3 = types.InlineKeyboardButton('Институт радиоэлектроники и информационных технологий - РтФ',
                                              callback_data='rtf')
    inline_btn_4 = types.InlineKeyboardButton('Институт строительства и архитектуры', callback_data='isa')
    inline_btn_6 = types.InlineKeyboardButton('Институт физической культуры, спорта и молодежной политики',
                                              callback_data='ipe')
...
async def init_bot(message: types.Message):
    ...
    await message.answer(
        "Привет 👋, я бот проверяющий ваше положение в конкурсном списке УРФУ среди тех, кто подал согласие.",
        reply_markup=markup)
...
async def get_text_messages(message):
    if message.chat.type == "private":
        if message.text == "Начать узнавать своё положение!" or message.text == "Изменить номер":
            ...
            await message.answer(
                "Расскажу о метсте в рейниге или про ФИИТ /fiit.👺 Используй клваиатуру для управления.")
...
async def process_callback_kb1btn1(callback_query: types.CallbackQuery):
    ...
    await bot_obj.edit_message_text(chat_id=callback_query.message.chat.id,
